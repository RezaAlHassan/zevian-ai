...
const EmployeeDetailPage: React.FC<EmployeeDetailPageProps> = ({
    ...
    if (viewMode === 'manager' && currentManagerId && !canView) {
        return (
            <div className="w-full px-6 py-6">
                <Button onClick={onBack} variant="outline" className="mb-4"><ArrowLeft className="mr-2 h-4 w-4" />Back to Employees
                </Button>
    ...
    const [dateRange, setDateRange] = useState<DateRange | undefined>({
        from: earliestReportDate,
        to: today
    });

    // Sync dateRange when startDate/endDate change (e.g. initially or externally)
    useEffect(() => {
    ...
    const handleDateRangeChange = (range: DateRange | undefined) => {
        setDateRange(range);
        if (range?.from) {
            setStartDate(toLocalDateString(range.from));
        }
        if (range?.to) {
            setEndDate(toLocalDateString(range.to));
        }
    };
    ...
    const columns: ColumnDef<Report>[] = [
        ...
        },
        {
            ...
            header: "Zevian Score",
            cell: ({ row }) => (
                <span className="capitalize text-muted-foreground">{(row.original.evaluationScore ?? 0).toFixed(1)}</span>
            )
        },
        {
    ...
    return (
        <div className="w-full px-6 py-6 space-y-6">
            <div className="sticky top-0 z-20 bg-card/90 backdrop-blur-md p-4 rounded-lg border border-border flex flex-col sm:flex-row gap-4 items-center -mx-4 mb-6">
                <button
                    onClick={onBack}
                ...
                <h2 className="text-xl font-bold text-foreground">Performance: {employee.name}</h2>
            ...
            <div className="bg-primary/10 border border-primary/30 rounded-lg p-4">
                ...
                {summary && (
                    <div className="bg-muted p-4 rounded-lg text-sm text-muted-foreground border border-border mt-4">
                        {summary}
                    </div>
                )}
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    ...
                    <div className="p-6 border-b border-border bg-muted/30">
                        <div className="flex items-center justify-between gap-4">
                            <div>
                                ...
                                <p className="text-sm text-muted-foreground mt-1">Holistic proficiency across projects</p>
                            </div>
                            <div className="flex items-center gap-2">
                                {viewMode === 'manager' && (
                                    <Button
                                        ...
                                        className="flex items-center gap-2"><Sliders className="mr-2 h-4 w-4" />Customize Metrics
                                    </Button>
                                )}
                            </div>
                        </div>
                        ...
                        {radarChartData.length > 0 ? (
                            <>
                                <div className="relative flex-1">
                                    ...
                                    <div className="bg-background rounded-xl p-6 border border-border h-full min-h-[400px]">
                            ...
                            <div className="flex-1 flex flex-col items-center justify-center py-12 text-center bg-muted/20 rounded-xl border border-dashed border-border">
                                <Star size={32} className="text-muted-foreground mb-3 opacity-20" />
                                ...
                                <Button
                                    ...
                                    className="mt-6 flex items-center gap-2 mx-auto"><Sparkles className="mr-2 h-4 w-4" />
                ...
                <div className="bg-card p-6 rounded-xl border border-border flex flex-col">
                    ...
                    {filteredReports.length > 1 ? (
                        <div className="flex-1 min-h-[400px] mt-6">
                            <ResponsiveContainer width="100%" height="100%">
                                ...
                                >
                                    ...
                                    />
                                    <Tooltip contentStyle={{ backgroundColor: 'hsl(var(--card))', border: '1px solid hsl(var(--border))', color: 'hsl(var(--card-foreground))', borderRadius: '0.5rem' }} />
                        ...
                        <div className="flex-1 flex flex-col items-center justify-center py-12 text-center bg-muted/20 rounded-xl border border-dashed border-border mt-6">
                            <TrendingUp size={32} className="text-muted-foreground mb-3 opacity-20" />
                ...
                </div>
                {
                    filteredReports.length > 0 ? (
                        ...
                        <div className="flex flex-col items-center justify-center py-12 text-center bg-muted/20 rounded-xl border border-dashed border-border">
                            <FileText size={32} className="text-muted-foreground mb-3 opacity-20" />
            ...
            >
                <div className="space-y-6">
                    <div className="flex items-center justify-between">
                        <p className="text-sm text-muted-foreground">Detailed breakdown of skills measured by Zevian AI based on all project reports.</p>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 max-h-[60vh] overflow-y-auto pr-2 custom-scrollbar">
                        {sortedSkills.map((skill, index) => (
                            <div key={index} className="bg-card p-4 rounded-xl border border-border flex flex-col gap-3 hover:border-primary/30 transition-all duration-200">
                                <div className="flex items-center justify-between">
                    ...
                    {viewMode === 'manager' && (
                        <div className="pt-4 border-t border-border flex justify-end gap-3">
                            <Button variant="outline" size="sm" onClick={() => setIsMetricsModalOpen(true)}><Sliders className="mr-2 h-4 w-4" />Customize Metrics
                            </Button>
                            <Button
                                size="sm"
                                onClick={() => {
                                ...
                                disabled={isAnalyzingSkills || filteredReports.length === 0}><Sparkles className="mr-2 h-4 w-4" />Re-analyze Skills
                            </Button>
                        </div>
                    )}
                </div>
