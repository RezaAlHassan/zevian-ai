...
const Header: React.FC<HeaderProps> = ({
  ...
  return (
    <header className="bg-background sticky top-0 z-50 border-b border-border h-16">
          ...
          {viewMode === 'employee' && (
            <div className="flex items-center gap-4">
              <button
                ...
                className="flex items-center gap-2.5 px-4 py-2 rounded-xl bg-primary text-primary-foreground font-semibold text-sm hover:bg-primary/90 transition-all active:scale-95"
              >
                <FileText size={16} />
              ...
              {nextReportDue && (
                <div className={`flex flex-col px-3 py-1.5 rounded-lg border transition-all ${nextReportDue.status.isOverdue ? 'bg-destructive/5 border-destructive/20 text-destructive' : 'bg-primary/5 border-primary/20 text-primary'}`}>
                  <div className="flex items-center gap-2">
                    {nextReportDue.status.isOverdue ? <AlertCircle size={14} className="animate-pulse" /> : <Clock size={14} />}
                    <span className="text-xs font-bold uppercase tracking-wider">{nextReportDue.status.label}</span>
                  </div>
        ...
        <div className="flex items-center gap-2 ml-auto">
          {viewMode === 'manager' && currentManagerId && (
            <Dropdown
              ...
              size="sm"
              align="right"
              buttonClassName="bg-muted border-border h-9"
            >
              ...
              <DropdownItem onClick={() => setScopeFilter('direct-reports')}>
                <div className="flex items-center gap-2 group/item">
                  <Users size={16} className={`${scopeFilter === 'direct-reports' ? 'text-primary scale-110' : 'text-muted-foreground group-hover/item:text-primary group-hover/item:scale-110'} transition-all duration-200`} />
              ...
              <DropdownItem onClick={() => setScopeFilter('reporting-chain')}>
                <div className="flex items-center gap-2 group/item">
                  <Network size={16} className={`${scopeFilter === 'reporting-chain' ? 'text-primary scale-110' : 'text-muted-foreground group-hover/item:text-primary group-hover/item:scale-110'} transition-all duration-200`} />
              ...
              {canViewOrgWide && (
                <DropdownItem onClick={() => setScopeFilter('organization')}>
                  <div className="flex items-center gap-2 group/item">
                    <Building2 size={16} className={`${scopeFilter === 'organization' ? 'text-primary scale-110' : 'text-muted-foreground group-hover/item:text-primary group-hover/item:scale-110'} transition-all duration-200`} />
